    ...
    for c = cont_vals
        ...
        for i = 1:length(imaging)
            ...
            if any(idx)
                ...
                for j = find(idx)
                    errors_u = abs(imaging(i).u_real - imaging(i).data(j).u_est);
        ...
        ylabel('Error U', 'FontSize', 12, 'FontWeight', 'bold');
        title('U-Component Errors vs Time per Image', 'FontSize', 14, 'FontWeight', 'bold');
        ...
        for i = 1:length(imaging)
            ...
            if any(idx)
                ...
                % Plot individual errors (lighter)
                for j = find(idx)
                    errors_v = abs(imaging(i).v_real - imaging(i).data(j).v_est);
        ...
        title('V-Component Errors vs Time per Image', 'FontSize', 14, 'FontWeight', 'bold');
        legend('Location', 'best', 'FontSize', 10);
        hold off;
        
        % Add overall title with blur/continuity info
        sgtitle(sprintf('Errors vs Time per Image | Blur: %s, Continuity: %s', blur_label, cont_label), ...
...
for i = 1:length(imaging)
    ...
    set(gca, 'XTick', 1:4, 'XTickLabel', blur_cont_combinations, 'XTickLabelRotation', 45);
    ylabel('Mean MAE U', 'FontSize', 10, 'FontWeight', 'bold');
    title(sprintf('%dx%d - U Component', imaging(i).resolution(1), imaging(i).resolution(2)), ...
    ...
    set(gca, 'XTick', 1:4, 'XTickLabel', blur_cont_combinations, 'XTickLabelRotation', 45);
    ylabel('Mean MAE V', 'FontSize', 10, 'FontWeight', 'bold');
    title(sprintf('%dx%d - V Component', imaging(i).resolution(1), imaging(i).resolution(2)), ...
          'FontSize', 11, 'FontWeight', 'bold');
    hold off;
end

sgtitle('Effect of Blur and Continuity on MAE by Resolution', ...
        'FontSize', 16, 'FontWeight', 'bold');
set(gcf, 'Color', 'w');
